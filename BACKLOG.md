...
- **[RESOLVED 2026-05-23, v1.5.0] chip.ogrady.golf post-Access auth was failing.** The Access app "Chip Leader" held a stale binding (most likely the deleted `chip-ogrady-golf` Worker as its service URL) so post-auth requests dead-ended at Cloudflare's edge and never reached the tunnel daemon. Fix: delete + recreate the Access app per `deploy/cloudflared/SETUP.md` §6. Verified end-to-end. See CHANGELOG v1.5.0.
